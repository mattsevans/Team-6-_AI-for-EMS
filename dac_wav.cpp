...
int main(int argc, char **argv) {
    ...
    // Playback gain (will be auto-computed from WAV stats)
    double playback_gain = 1.0;

    std::cout << "Loaded WAV: " << wavPath << "\n";
    std::cout << "  Sample rate: " << wav.sampleRate << " Hz\n";
    std::cout << "  Samples:     " << wav.samples.size() << "\n";

        // ---- WAV stats + auto playback gain ----
    if (!wav.samples.empty()) {
        ...
        double peak_norm = std::max(std::abs(static_cast<int>(min_s)),
                                    std::abs(static_cast<int>(max_s))) / 32768.0;
        double rms = std::sqrt(sum_sq / wav.samples.size());

        std::cout << "WAV stats:\n";
        std::cout << "  min sample: " << min_s << "\n";
        std::cout << "  max sample: " << max_s << "\n";
        std::cout << "  peak norm:  " << peak_norm << "\n";
        std::cout << "  RMS norm:   " << rms << "\n";

        // Auto-normalize: target ~90% of full-scale at the DAC
        const double target_peak = 0.9;
        if (peak_norm > 0.0) {
            playback_gain = target_peak / peak_norm;

            // Don't attenuate if the file is already loud
        ...
        double user_boost = 2.0;   // try 2x, then 3x, etc.
        playback_gain *= user_boost;
        std::cout << "  Auto playback gain: " << playback_gain << "\n";
    } else {
        std::cout << "WAV stats: no samples loaded.\n";
        playback_gain = 1.0;
    }
    // ----------------------------------------
    // --------------------------------------------
    ...
    // NEW: track how many samples get clipped
    ...
    for (size_t n = 0; n < totalSamples && !g_stop; ++n) {
        ...
        // Apply auto playback gain
        x *= playback_gain;

        // NEW: Clip to avoid exceeding DAC range AND count clips
        if (x > 1.0) {
            x = 1.0;
            clipped_count++;
        } else if (x < -1.0) {
    ...
    // NEW: print clipping stats after playback loop
    if (totalSamples > 0) {
        double pct = 100.0 * static_cast<double>(clipped_count)
                               / static_cast<double>(totalSamples);
        std::cout << "Clipped samples: " << clipped_count
                  << " (" << pct << "% of " << totalSamples << ")\n";
    }
