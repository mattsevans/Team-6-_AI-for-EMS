...
from post_whisper_translate import (
...
VAD_HANGOVER_MS = 550        # keep speech active this long after last voiced frame
...
SPK_SIM_THRESHOLD = 0.52     # cosine similarity threshold to create new speaker
...
WHISPER_CPP_BIN = os.environ.get("WHISPER_CPP_BIN", "whisper.cpp/build/bin/Release/whisper-cli")
WHISPER_CPP_MODEL = os.environ.get("WHISPER_CPP_MODEL", "whisper.cpp/ggml-base.bin")
...
def load_glossary(path: Path = GLOSSARY_PATH) -> List[str]:
    try:
        # Implement your CSV/JSON loading here if you want to bias prompts later.
        return []
    except Exception:
        return []
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    # Assemble text from segments (robust to format variants)
                    transcription = data.get("transcription")
                    text = " ".join(s.get("text","").strip() for s in transcription)
                    #Debug lines
                    # print("DEBUG keys:", list(data.keys()))
                    #print("DEBUG num_segments:", len(data.get("segments", [])))
                    #if data.get("segments"):
                    #    print("DEBUG first_segment_text:", repr(data["segments"][0].get("text")))
                    #print("DEBUG top_text:", repr(data.get("text")))
                    ...
                    if text:
                        ...
                        if job.speaker_id == 1:
                            line = f"[{dt:%H:%M:%S}] EMS: {text}"
                        else: 
                            line = f"[{dt:%H:%M:%S}] Speaker{job.speaker_id-1}: {text}"
                        safe_print(line)
                        append_jsonl(line)
...
INBOX_PATH = os.environ.get("INBOX_PATH", "inbox.jsonl")
        ...
        while not self._stop.is_set():
            try:
                ...
                if size > self._file_offset:
                    ...
                    for raw in lines[:-1]:
                        text = raw.decode(self._enc, errors="replace").strip()
                        if text:
                            self._q.put(text)
                            print(f"TTS: queued (+1) len={len(text)} queue={self._q.qsize()}")
...
def main():
    ...
    tts = TTSQueue(engine=os.environ.get("TTS_ENGINE","espeak-ng"),
                speed=int(os.environ.get("TTS_SPEED_WPM","170")),
                safe_print=safe_print)
    translator = Translator(impl=os.environ.get("TRANSLATION_LIB","argos"),
                            safe_print=safe_print)
    postproc = PostWhisperProcessor(state, tts, translator, safe_print=safe_print)
